...
- (void)drawDots
{
    ...
    arrDotXPoints =[[NSMutableArray alloc]init];
    // Remove all dots that were previously on the graph
...
#pragma clang diagnostic pop
            ...
            [arrDotXPoints addObject:[NSNumber numberWithFloat: positionOnXAxis]];
...
- (void)drawYAxis
{
    ...
    if (self.positionYAxisRight)
    {
        ...
        //frameForLabelYAxis = CGRectMake(self.frame.size.width - self.YAxisLabelXOffset - 5, 0, self.YAxisLabelXOffset , 15);
        frameForLabelYAxis = CGRectMake(self.frame.size.width - self.YAxisLabelXOffset - 5, 0, self.YAxisLabelXOffset , 15);
    ...
    else
    {
        ...
        frameForLabelYAxis = CGRectMake(0, 0, self.YAxisLabelXOffset , 15);
...
- (void)drawXAxisNew
{
    ...
    for (int i=0; i<arrDotXPoints.count; i++)
    {
        NSString *firstXLabel = [self xAxisTextForIndex:i];
        ...
        UILabel *firstLabel = [self xAxisLabelWithText:firstXLabel atIndex:0];
        //firstLabel.frame = CGRectMake([[xAxisLabelPoints objectAtIndex:i] floatValue], self.frame.size.height-100, 10, 100);
        if (i==0)
        {
            firstLabel.frame = CGRectMake([[arrDotXPoints objectAtIndex:i] floatValue]-29, self.frame.size.height-35, 100, 15);
        }
        else if((i+1)==arrDotXPoints.count)
        {
            firstLabel.frame = CGRectMake([[arrDotXPoints objectAtIndex:i] floatValue]-38, self.frame.size.height-35, 100, 15);
        }
        else
        {
            firstLabel.frame = CGRectMake([[arrDotXPoints objectAtIndex:i] floatValue]-35, self.frame.size.height-35, 100, 15);
        }
       
        
        [firstLabel setTransform:CGAffineTransformMakeRotation(-M_PI / 3)];
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint
{
    ...
    //commented (as it always changing pop lable)
    /*
    if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
    ...
    */
    ...
    if (!self.usingCustomPopupView)
    {
        ...
        // to add date on point click
        NSString *strDate = [self xAxisTextForIndex:index];
        
        NSNumber *value = dataPoints[index];
        NSString *formattedValue = [NSString stringWithFormat:self.formatStringForValues, value.doubleValue ,strDate];
        
        //formattedValue = [NSString stringWithFormat:@"%@ On %@", formattedValue ,strDate];
        suffix=[NSString stringWithFormat:@" on %@", strDate];
        [self.popUpLabel sizeToFit];
        ...
        if (index==arrDotXPoints.count-1)
        {
            self.popUpLabel.frame=CGRectMake(self.popUpLabel.frame.origin.x-20, self.popUpLabel.frame.origin.y, self.popUpLabel.frame.size.width, self.popUpLabel.frame.size.height);
        }
